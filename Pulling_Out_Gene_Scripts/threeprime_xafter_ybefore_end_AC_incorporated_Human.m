...
annotationloaddir = ...
  '/data/STUDENT_PLAY_AREA/TRANSCRIPTION_DATA/GENCODE/';
...
loadfile1 = 'GENCODE_basic_processed_positions.mat';
%gene positions for human genome
...
loadfile2 = 'GENCODE_basic_processed_ids.mat' ;
...
loadfile3 = 'GENCODE_basic_processed_positions_for_nogene_mean.mat';
...
NETseq_sample1 = importdata([dataloaddir 'proudfoot_HeLa_mNETseq.mat']);
%NETseq_sample2 = importdata([dataloaddir 'proudfoot_HeLa_mNETseq.mat']);

disp ('Loaded in Proudfoot HeLa mNETseq data')
...
%% S. CEREVISIAE CONTENT IGNORE could remove at some point but useful as template for now
%Start with WT 
% 
...
for i = 1:NoChromosomes
    ...
    gctr = size(gene_positions{i,1}(:,1), 1) ;
...
disp('Constructed NETseq matrices')
...
disp('plotted metagenes')
%% combine reverse and forward matrices
NET_Seq_Matrix = vertcat(NET_Seq_matrix_forward,NET_Seq_matrix_reverse) ;
disp('Combined NETseq matrices')
...
genome_size = 0 ;
%%below is protocol for two samples as for ulku data 
...
%     end
% end
%% calculate mean from just one sample 
for cctr = 1:NoChromosomes
    for sctr = 1:2
        total_across_genome = total_across_genome + sum(NETseq_sample1{cctr, sctr}) ;
        
        genome_size = genome_size + sum(numel(NETseq_sample1{cctr, sctr})) ;
        
        
    end
end
mean_across_genome = total_across_genome ./ genome_size ;
disp('calculated whole genome mean')
...
NETseq_nogenes = NETseq_sample1; %NETseq_composite if two samples are averaged 
...
for cctr = 1:1:NoChromosomes
    ...
    for gctr = size(gene_positions_for_nogene_mean{cctr,sctr}, 1):-1:1 %need to input before removal of genes by overlaps etc 
        ...
        %gets to the line above and takes ages, hour plus. Need to rethink
        %background for mean in humans. 
...
%mean_across_genome_nogene = 0.2160 %answer from wild type NETseq data YEAST 
mean_across_genome_nogene = total_across_genome_nogene ./ genome_size ;
disp('calculated nogene whole genome mean')
...
disp('removed low expression genes') 
...
%     
% end
disp('Normalised Binned NETseq matrix')
