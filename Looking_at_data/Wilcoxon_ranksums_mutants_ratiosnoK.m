...
% dataloaddirr = '/home/orie3770/Current_Cluster_Job93/';
% loadfile = 'NETseqWTratios.mat';

%Change the WT for PROseq still using NETseq clusters
dataloaddirr = '/home/orie3770/Current_Cluster_Job98/';
loadfile = 'PROseqWTratios.mat';
...
dataloadirr2 = '/home/orie3770/Current_Cluster_Job99/'%change X to mutant of interest ratios.
...
for clctr = 2:3
    ...
    if clctr == 2
        ...
        colnames(w,1) = append('WT',string(clctr-1),'vs ', mutant, string(clctr-1));
        ...
        PrePolyA_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PrePolyA_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr),'vs ', mutant, string(clctr));
    end
    ...
    if clctr == 3
        ...
        if clctr ==3
            ...
            colnames(w,1) = append('WT',string(clctr-2),' vs ', mutant, string(clctr-2));
        end
    end
    ...
    uicontrol('Style', 'text', 'Position', [20 100 200 20], 'String', 'Pre_PolyA_ranksums')
    ...
    if clctr ==2
        [p, h] = ranksum(termination_ratio_matrix_WT(classes2clust==(clctr-1)& Removetopbottom10pclogical2_WT == 1 ), termination_ratio_matrix(classes2clust == (clctr-1) & Removetopbottom10pclogical2 == 1 ));
        w=w+1;
        ...
        [p, h] = ranksum(termination_ratio_matrix_WT(classes2clust==(clctr)& Removetopbottom10pclogical2_WT == 1 ), termination_ratio_matrix(classes2clust == clctr & Removetopbottom10pclogical2 == 1 ));
        w=w+1;
        PostPolyA_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostPolyA_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr),' vs ', mutant, string(clctr));
    ...
    if clctr ==3
        [p, h] = ranksum(termination_ratio_matrix_WT(classes3clust==(clctr-1)& Removetopbottom10pclogical2_WT == 1 ), termination_ratio_matrix(classes3clust == (clctr-1) & Removetopbottom10pclogical2 == 1 ));
        w=w+1;
        PostPolyA_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostPolyA_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr-1),' vs ', mutant, string(clctr-1));
        
        [p, h] = ranksum(termination_ratio_matrix_WT(classes3clust==(clctr)& Removetopbottom10pclogical2_WT == 1 ), termination_ratio_matrix(classes3clust == clctr & Removetopbottom10pclogical2 == 1 ));
        w=w+1;
        PostPolyA_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostPolyA_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr),' vs ', mutant, string(clctr));
        if clctr ==3
            [q, i] = ranksum(termination_ratio_matrix_WT(classes3clust==(clctr-2) & Removetopbottom10pclogical2 == 1), termination_ratio_matrix(classes3clust == (clctr-2) & Removetopbottom10pclogical2 == 1));
            w=w+1;
    ...
    z.Position(3:4) = z.Extent(3:4);
    uicontrol('Style', 'text', 'Position', [20 100 200 20], 'String', 'Post_PolyA_ranksums')
    %% save table and textfile
    ...
    if clctr ==2
        [p, h] = ranksum(aftermath_ratio_matrix_WT(classes2clust==(clctr-1)& Removetopbottom10pclogical3_WT == 1 ), aftermath_ratio_matrix(classes2clust == (clctr-1) & Removetopbottom10pclogical3 == 1 ));
        w=w+1;
        ...
        [p, h] = ranksum(aftermath_ratio_matrix_WT(classes2clust==(clctr)& Removetopbottom10pclogical3_WT == 1 ), aftermath_ratio_matrix(classes2clust == (clctr) & Removetopbottom10pclogical3 == 1 ));
        w=w+1;
        PostTerm_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostTerm_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr),' vs ', mutant, string(clctr));
    ...
    if clctr ==3
        [p, h] = ranksum(aftermath_ratio_matrix_WT(classes3clust==(clctr-1)& Removetopbottom10pclogical3_WT == 1 ), aftermath_ratio_matrix(classes3clust == (clctr-1) & Removetopbottom10pclogical3 == 1 ));
        w=w+1;
        PostTerm_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostTerm_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr-1),' vs ', mutant, string(clctr-1));
        ...
        [p, h] = ranksum(aftermath_ratio_matrix_WT(classes3clust==(clctr)& Removetopbottom10pclogical3_WT == 1 ), aftermath_ratio_matrix(classes3clust == (clctr) & Removetopbottom10pclogical3 == 1 ));
        w=w+1;
        PostTerm_ratio_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        PostTerm_ratio_ranksums(2, w) = h;
        colnames(w,1) = append('WT',string(clctr),' vs ', mutant, string(clctr));
        if clctr ==3
            [q, i] = ranksum(aftermath_ratio_matrix_WT(classes3clust==(clctr-2) & Removetopbottom10pclogical3_WT == 1), aftermath_ratio_matrix(classes3clust == (clctr-2) & Removetopbottom10pclogical3 == 1));
            w=w+1;
    ...
    z.Position(3:4) = z.Extent(3:4)
    uicontrol('Style', 'text', 'Position', [20 100 200 20], 'String', 'Post_Term_ranksums');
...
%%
system(append('cd Mutant_Ratio_Wilcoxon_Ranksums; mkdir ', mutant, datatype1,'corrected'))
%%

%%
unix(append('mv *ranksum* ./Mutant_Ratio_Wilcoxon_Ranksums/', mutant, datatype1,'corrected'))
