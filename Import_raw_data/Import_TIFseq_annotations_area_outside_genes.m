...
for ictr=1:1:length(unique_IDs)
    ...
    if ~strcmp(unique_IDs{ictr},'NA')
        ...
        if (sum(temp_ypd_vals) > 0)
            ...
            if (temp_strand == 1)
                gene_positions_for_nogene_mean{temp_chr,temp_strand}(chromosome_counter(temp_chr,temp_strand),:) = ...
...
gene_positions_old = gene_positions_for_nogene_mean;
...
for cctr=1:1:NoChromosomes
   ...
   for sctr=1:2
       ...
       [new_starts,new_positions_idx] = ...
           sort(gene_positions_for_nogene_mean{cctr,sctr}(:,1));
       
       new_ends = gene_positions_for_nogene_mean{cctr,sctr}(new_positions_idx,2);
       ...
       gene_positions_for_nogene_mean{cctr,sctr} = [new_starts, new_ends];
       ...
       if ( sum( ...
               (gene_positions_for_nogene_mean{cctr,sctr}(2:end,1) - ...
               gene_positions_for_nogene_mean{cctr,sctr}(1:end-1,2)) < 0 ) ~= 0)
           disp([num2str(sum( ...
               (gene_positions_for_nogene_mean{cctr,sctr}(2:end,1) - ...
               gene_positions_for_nogene_mean{cctr,sctr}(1:end-1,2)) < 0 )) ...
       ...
       if ( sum( ...
               (gene_positions_for_nogene_mean{cctr,sctr}(:,2) - ...
               gene_positions_for_nogene_mean{cctr,sctr}(:,1)) < 0 ) ~= 0)
...
% 
save([dataloaddir 'gene_positions_pelechano_TIFseq_for_nogene_mean.mat'],'gene_positions_for_nogene_mean')
% save([dataloaddir 'gene_names_pelechano_TIFseq.mat'],'gene_names')
% save([dataloaddir 'gene_types_pelechano_TIFseq.mat'],'gene_types')
% 
% 
% %% Notes:
% 
