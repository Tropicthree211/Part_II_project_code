...
minGeneSize = 500; %GM changed to 500 for more signal
...
for cctr=1:1:NoChromosomes
    ...
    for gctr=1:1:size(gene_positions{cctr,strand},1)
        ...
        %Also check for overlaps with the opposing strand here
        %GM to remove cramped convergent ends should edit here too. 
        for ictr = 1:size(gene_positions{cctr,3-strand},1)
           if( (gene_positions{cctr,strand}(gctr,2) >= (gene_positions{cctr,3-strand}(ictr,1)-500)) && ... %GM -500 to remove cramped convergent opposing strand genes. 
                   (gene_positions{cctr,strand}(gctr,1) <= gene_positions{cctr,3-strand}(ictr,2)) )
               remove_genes{cctr,strand}(gctr) = 1;
               remove_genes{cctr,3-strand}(ictr) = 1; %GM if genes converging then want to remove both as both may have 3' end perturbed by other's interference. 
           end
        end
...
save([dataloaddir 'gene_positions_pelechano_TIFseq_no500paraconv.mat'],'gene_positions')
save([dataloaddir 'gene_names_pelechano_TIFseq_no500paraconv.mat'],'gene_names')
save([dataloaddir 'gene_types_pelechano_TIFseq_no500paraconv.mat'],'gene_types')
