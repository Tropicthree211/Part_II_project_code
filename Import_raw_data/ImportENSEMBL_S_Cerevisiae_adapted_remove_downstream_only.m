...
minGeneSize = 1000; %GM changed min gene size as don't need large genes to visualise termination event and can get more signal
...
for cctr=1:1:NoChromosomes
    ...
    for gctr=1:1:size(gene_positions{cctr,strand},1)
        ...
        for ictr = (gctr+1):size(gene_positions{cctr,strand},1)
            if( (gene_positions{cctr,strand}(gctr,2) >= gene_positions{cctr,strand}(ictr,1)) && ...%GM added -500 to remove genes with another gene within 500bp from end 
    ...
    for gctr=1:1:size(gene_positions{cctr,strand},1)
        ...
        for ictr = (gctr+1):size(gene_positions{cctr,strand},1)
            if( (gene_positions{cctr,strand}(gctr,2) >= gene_positions{cctr,strand}(ictr,1)) && ...%old GM -500 removes genes with another gene close downstream
                    (gene_positions{cctr,strand}(gctr,1) <= gene_positions{cctr,strand}(ictr,2)) ) %GM now removing overlaps first then removing downstream nearby genes later
                remove_genes{cctr,strand}(gctr) = 1;
                remove_genes{cctr,strand}(ictr) = 1;
            end            
        end
...
%% Remove genes with another 500bp or less donwstream 
% have duplicated the above code to apply two separate screens, the second
% of which removes only the downstream gene not both involved 
...
for cctr=1:1:NoChromosomes
    ...
    for gctr=1:1:size(gene_positions{cctr,strand},1)
        ...
        for ictr = (gctr+1):size(gene_positions{cctr,strand},1)
            if( (gene_positions{cctr,strand}(gctr,2) >= gene_positions{cctr,strand}(ictr,1)-500) && ...%GM added -500 to remove genes with another gene within 500bp from end 
                    (gene_positions{cctr,strand}(gctr,1) <= gene_positions{cctr,strand}(ictr,2)) )
                remove_genes{cctr,strand}(gctr) = 1;
                remove_genes{cctr,strand}(ictr) = 0; %GM modified to remove only the gene with the close adjacent gene not the one that is close.
            end            
        end
        %Also check for overlaps with the opposing strand here %DONE ABOVE
%         for ictr = 1:size(gene_positions{cctr,3-strand},1)
%            if( (gene_positions{cctr,strand}(gctr,2) >= gene_positions{cctr,3-strand}(ictr,1)) && ...
%                    (gene_positions{cctr,strand}(gctr,1) <= gene_positions{cctr,3-strand}(ictr,2)) )
%                remove_genes{cctr,strand}(gctr) = 1;
...
%         end
    ...
    for gctr=1:1:size(gene_positions{cctr,strand},1)
        ...
        for ictr = (gctr+1):size(gene_positions{cctr,strand},1)
            if( (gene_positions{cctr,strand}(gctr,2) >= gene_positions{cctr,strand}(ictr,1)-500) && ...%GM -500 removes genes with another gene close downstream
                    (gene_positions{cctr,strand}(gctr,1) <= gene_positions{cctr,strand}(ictr,2)) )
                remove_genes{cctr,strand}(gctr) = 1;
                remove_genes{cctr,strand}(ictr) = 0;%GM modified to remove only the gene with the close adjacent gene not the one that is close.
            end            
...
%% Save the imported and processed gene positions
 ...
 save([dataloaddir 'Directly_downstream_removed_gene_positions_Saccharomyces_cerevisiae__R64_1_1__101.mat'],'gene_positions')
