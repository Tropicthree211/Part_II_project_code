...
% %% Pull out nonprotein coding genes 
...
doublefiltered_gene_names = gene_names_list(list_remaining_genes);
save(append(mutant1,'_actively_expressing_genes_list', annotations, 'noXuts' , '.mat'),'list_remaining_genes') %must use same annotations file in both datasets if this is to work. 
...
Normalised_NETseq_Bins_MatrixNET = Normalised_NETseq_Bins_Matrix;
%%
save(append('/home/orie3770/Modelling/', 'exp_modeled_UU', mutant1, 'NETseq_profiles', string(Current_modelling_job), '.mat'), ...
'mindistprofiles', 'mindistvalues', 'Normalised_NETseq_Bins_MatrixNET') %change name before saving to avoid overwriting
...
%  legend(l1) %,l2) 
% 
% filename = append(mutant1, datatype, 'expdropmodelled300b300a',string(n));
% % mutant, datatype1
% title(filename)
...
NETseq_matrix_no_negative = NETseq_matrix_no_negative(list_remaining_genes, :);
...
srand = s(randperm(numel(s),10000));
mrand = m(randperm(numel(m),10000));


smval = [srand mrand]; 
%% Coding 2nd model using the parameters 
profile_matrix2 = zeros(size(smval, 1), size(Normalised_NETseq_Bins_Matrix, 2));
for smplctr = 1:1: size(profile_matrix2, 1)
    for x = 1:size(profile_matrix2,2)        
            
            profile_matrix2(smplctr, x) = (1/smval(smplctr, 1)*(6.3)^(1/2))*exp(-1/2*((x-smval(smplctr, 2)/smval(smplctr,1))^2)); 
            %will replace letters above with indexes to the parameter
            %matrix. 
...
%% compare each gene to each profile find minimum squared euclidean distance profile  
mindistprofiles2 = zeros(size(Normalised_NETseq_Bins_Matrix,1), size(Normalised_NETseq_Bins_Matrix,2));
mindistvalues2 = zeros(size(Normalised_NETseq_Bins_Matrix,1), 1);

dist_profile = zeros(size(profile_matrix2, 1), 1);


for gctr = 1:1:size(Normalised_NETseq_Bins_Matrix, 1)
    
    for tmpctr = 1:1: size(profile_matrix2, 1)
%         geneplusprofile = vertcat(Normalised_NETseq_Bins_Matrix(gctr, :), profile_matrix(tmpctr, :));
        
       
%         dist_profile(tmpctr ,1) = mean(pdist(geneplusprofile, 'squaredeuclidean'));
          dist_profile(tmpctr ,1) = mean(pdist2(Normalised_NETseq_Bins_Matrix(gctr, :),profile_matrix2(tmpctr, :) , 'squaredeuclidean')); 
          %should run faster if not vertcating every run
    end
    [mindistvalues2(gctr,1) , I] = min(dist_profile); 
    mindistprofiles2(gctr, :) = profile_matrix2(I, :);
   
end

Normalised_NETseq_Bins_MatrixPRO = Normalised_NETseq_Bins_Matrix ;
%%
save(append('/home/orie3770/Modelling/', 'norm_modeled_LIS', mutant1, 'PROseq_profiles', string(Current_modelling_job), '.mat'), ...
'mindistprofiles2', 'mindistvalues2', 'Normalised_NETseq_Bins_MatrixPRO') %change name before saving to avoid overwriting
...
% seeing if it works 
%load('/home/orie3770/Modelling/exp_modeled_UUGlugal15NETseq_profiles13.mat')
%load(append('/home/orie3770/Modelling/', 'norm_modeled_LIS ', mutant1, 'PROseq_profiles', string(Current_modelling_job)))

%% find the peak 
tic
normpeakclasses = zeros(size(Normalised_NETseq_Bins_Matrix, 1),1 ); % matrix in which exponential drop location class will be stored.
for gctr =  1:1:size(Normalised_NETseq_Bins_Matrix, 1) % go through each gene in turn
    for pctr = 2:1:size(Normalised_NETseq_Bins_Matrix, 2) % go through each position in the gene
        if max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (2<=pctr) && (pctr<6) 
            normpeakclasses(gctr, 1) = 1 ;
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (6<=pctr) && (pctr<10)
            normpeakclasses(gctr, 1) = 2 ;            
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (10<=pctr) && (pctr<14)
            normpeakclasses(gctr, 1) = 3 ;
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) &&(14<=pctr) && (pctr<18)
            normpeakclasses(gctr, 1) = 4 ;
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) &&(18<=pctr) && (pctr<22)
            normpeakclasses(gctr, 1) = 5 ;
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (22<=pctr) && (pctr<26)
            normpeakclasses(gctr, 1) = 6 ;
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (26<=pctr) && (pctr<30)
            normpeakclasses(gctr, 1) = 7 ;
            
            %extend size of area with more elseifs
            
       elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (30<=pctr) && (pctr<34)
            normpeakclasses(gctr, 1) = 8 ;
       
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (34<=pctr) && (pctr<38)
            normpeakclasses(gctr, 1) = 9 ;  
       
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (38<=pctr) && (pctr<42)
            normpeakclasses(gctr, 1) = 10; 
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (42<=pctr) && (pctr<46)
            normpeakclasses(gctr, 1) = 11; 
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (46<=pctr) && (pctr<50)
            normpeakclasses(gctr, 1) = 12; 
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (50<=pctr) && (pctr<54)
            normpeakclasses(gctr, 1) = 13;     
            
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (54<=pctr) && (pctr<58)
            normpeakclasses(gctr, 1) = 14;   
          
        elseif max(mindistprofiles2(gctr,:)) == mindistprofiles2(gctr, (pctr)) && (58<=pctr) && (pctr<60)
            normpeakclasses(gctr, 1) = 15;   
        end
    end
end
toc                        
...
filtered_gene_names = gene_names_list(list_remaining_genes);
 ...
 %% plot all normpeakclasses overlayed with mutants
%  
% for n= 1:15
% 
% metagene=nanmean(Normalised_NETseq_Bins_Matrix(normpeakclasses == n, :));
% figure
% plot(metagene)
% %legend(append(mutant, string(size(Normalised_NETseq_Bins_Matrix(expdropclasses == n, :)))));
% hold ON
% mutmetagene=nanmean(NETseq_3exprn_selected_ufdata(normpeakclasses == n, :));
% plot(mutmetagene)
% 
% l1 = append(mutant1, datatype,' ', string(size(Normalised_NETseq_Bins_Matrix(normpeakclasses == n), 1)))
% l2 =append(mutant, datatype1,' ',  string(size(Normalised_NETseq_Bins_Matrix(normpeakclasses == n), 1)))
% 
% legend(l1,l2) 
% 
% filename = append(mutant1, datatype, mutant, datatype1, 'normpeakmodelled300b300a',string(n));
% 
% title(filename)
% saveas(gcf, filename, 'jpeg')
% saveas(gcf, filename, 'fig')
% 
% %save gene names alongside 
% expdrop_gene_names = filtered_gene_names(normpeakclasses == n);
% directory = '/home/orie3770/';
% fid = fopen(append(directory, filename , 'names','.txt' ) , 'w' );
% fprintf(fid, '%s\n', expdrop_gene_names);
% fclose(fid);
...
% mindistmeans = zeros(15,1) ;
% for n = 1:15
% mindistmeans(n,1) = mean(mindistvalues(normpeakclasses == n));
% end
% figure
% plot(mindistmeans)
% title('mean minimum squared euclidean distance value')
...
% unix(append('mv ',mutant1, datatype, 'nonproteincodinggenes','*', ' ./Current_Modelling_Job', string(Current_modelling_job)))
% %%
% unix(append('mv ',mutant1, datatype, mutant, datatype1, 'normpeakmodelled300b300a' , '*', ' ./Current_Modelling_Job', string(Current_modelling_job)))
% %
toc
...
%% Got expdrops and normpeaks on same genes now compare 

compareends = [normpeakclasses expdropclasses];

difference = compareends(:,1) - compareends(:,2);
histogram(difference)
title('Histogram of difference between modeled PROseq peak location and NETseq drop location')
filename = append(mutant1, datatype, mutant, datatype1, 'modelled');
saveas(gcf, append(filename, 'peakcomparisonhistogram'), 'fig')
saveas(gcf, append(filename, 'peakcomparisonhistogram'), 'jpeg') 

unix(append('mkdir Current_Modelling_Job', string(Current_modelling_job)))
unix(append('mv ', mutant1, datatype, mutant, datatype1, '* Current_Modelling_Job', string(Current_modelling_job)))


%% make metagenes pulling out difference genes, might be interesting 
%fully expect this to be a hot mess but who knows something interesting
%might be pulled out.
%hypothesis: does the profile of genes remain similar with different PROseq
%and NETseq peak locations?
for n = -15:1:15
differencesmetagene = nanmean(Normalised_NETseq_Bins_MatrixNET(difference == n, :));

figure 
plot(differencesmetagene)
title(append('NETseq and PROseq peaks diverging', string(n)));
hold ON 



differencesmetagene2 = nanmean(Normalised_NETseq_Bins_MatrixPRO(difference == n,:));
plot(differencesmetagene2)
l1 = append(mutant1, datatype1, string(size(Normalised_NETseq_Bins_MatrixNET(difference == n),1)));
l2 = append(mutant, datatype, string(size(Normalised_NETseq_Bins_MatrixPRO(difference == n),1)));
legend(l1,l2)
%saveas(gcf, append(filename, 'differences_metagene', string(n)))
saveas(gcf, append(filename, 'differences_metagene', string(n)), 'jpeg')
%savsaveas(gcf, append(filename, 'differences_metagene', string(n)))e gene names alongside %j
difference_gene_names = filtered_gene_names(difference == n);
directory = '/home/orie3770/';
fid = fopen(append(directory, filename, string(n), 'names','.txt' ) , 'w' );
fprintf(fid, '%s\n', difference_gene_names);
fclose(fid);
