...
%every other too big 
%% Create the matrix containg all the different permutations of ABaw
%
Arep = repmat(A, size(B,1), 1);
Brep = repelem(B, size(A,1) , 1);
ApermB = [Arep Brep];
...
%
save(append('/home/orie3770/Modelling/', 'exp_modeled_UUWTNETseq_profiles', str(Current_modelling_job), '.mat'), ...
'mindistprofiles') %change name before saving to avoid overwriting
toc
...
% seeing if it works 
load('/home/orie3770/Modelling/exp_modeled_UUWTNETseq_profiles.mat')
...
%        
%         elseif mindistprofiles(gctr,pctr) < mindistprofiles(gctr, (pctr-1)) && (38<=pctr) && (pctr<=40)
...
toc                        
disp('drops found')
%% Use expdropclass to make 7 metagenes
...
legend(str(size(expdropclass1,1)))
...
legend(str(size(expdropclass12,1)))
...
legend(str(size(expdropclass3,1)))
...
legend(str(size(expdropclass4,1)))
...
legend(str(size(expdropclass5,1)))
...
legend(str(size(expdropclass6,1)))
...
legend(str(size(expdropclass7,1)))
...
metagene_exp8 = metagene_exp8 ./size(expdropclass7,1) ;
figure 
plot(metagene_exp8)
...
legend(str(size(expdropclass9,1)))
...
unix(append('mv modelling_* ./Current_Modelling_Job', string(Current_modelling_job)))
%
toc