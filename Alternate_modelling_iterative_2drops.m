...
qmatrix = zeros(size(Normalised_NETseq_Bins_Matrix, 1),1); 

for gctr = 1:1:size(Normalised_NETseq_Bins_Matrix, 1) 
diff1 = Normalised_NETseq_Bins_Matrix(gctr, 1:end-1) - ... %changed to -1 not sure if -5 was allowing noise to skew model
Normalised_NETseq_Bins_Matrix(gctr, 2:end) ; 
...
equation = ['a*heaviside(x-', num2str(p), ')*exp(-b*(x-', num2str(p), '))+d*heaviside(x-', num2str(q), ')*exp(-f*(x-', num2str(q), '))+c'];

%% Fitting the data 
st_ = [0.5 0.5 0 0.5 0.5]; 
...
%% Save the fit array 
save(append('/home/orie3770/Modelling/Alternative_modelling_', mutant1, datatype,string(Current_modelling_job), '.mat'), 'fitarray', '-mat');

%% Pull out genes to do metagene analysis
