...
gene_ids = gene_names; % the code below works in ids not names 
%moved to after loading the files in, would be a good idea lol
...
for cctr = 1:NoChromosomes
    ...
    for gctr = 1:x
        ...
        w = w + 1  ;
       if chromosomej_endpoints(gctr,1)<0
           disp('skipped a gene that was too close to chromosome edge')
          continue;   %Skips gene if the either side of pAS goes over end of chromosome
       end
...
for cctr = 1:NoChromosomes
    ...
    for gctr = 1:x
        ...
        w = w + 1  ;                                            
        if chromosomej2_endpoints(gctr,1)<0
            disp('skipped a gene that was too close to chromosome edge')
            continue;   %Skips gene if the either side of pAS goes over end of chromosome
        end
        %add to total signal matrix for all genes
...
%The above is valid for when I'm not filtering out all the genes before loading into this code 
% but
% since a lot of signal is getting removed by the nogene500pc filter cannot
% use this here. I had already specified a separate gene-positions for
% nogene mean so can just comment out and code will still run.
...
%unix(append('mv PROsppend('mv ', mutant, datatype1, '* ', JOB))eq', mutant1, '* ', JOB))
