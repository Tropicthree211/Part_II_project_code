...
%time 
%% Combine two Normalised NETseq binned matrices for clustering 
Normalised_NETseq_Bins_MatrixC = vertcat(Normalised_NETseq_Bins_Matrix, Normalised_NETseq_Bins_Matrix2) ;
...
clusters = append(datatype, mutant1, datatype2, mutant2, 'combined', annotations)
%% evaluate clusters calinskiHarabasz
eva = evalclusters(Normalised_NETseq_Bins_MatrixC ,myfunc,'CalinskiHarabasz','Klist',klist) 
figure 
...
eva = evalclusters(Normalised_NETseq_Bins_MatrixC ,myfunc,'silhouette','Klist',klist) 
figure
plot(eva.CriterionValues)
...
for k_temp=1:dim(1)
    [~,~,sumd]=kmeans(Normalised_NETseq_Bins_MatrixC,k_temp,'emptyaction','drop');
...
%clusters = append(datatype, mutant1, datatype2, mutant2, 'combined' ,
%annotations) %moved cluster description earlier 
...
%ufdata = 'PROseqspt4del';
 for K= 2:4
    for repeat = 1:5
        classes=kmeans(Normalised_NETseq_Bins_MatrixC ,K);
        
        %% Pull out the classes from the clustering
        Class_sorted_NET_seq_matrix = zeros(size(Normalised_NETseq_Bins_MatrixC, 1), size(Normalised_NETseq_Bins_MatrixC, 2));
        Classmetagenes = zeros(size(Normalised_NETseq_Bins_MatrixC, 2),K) ;
        ...
        for n =1:K
            ...
            Classn = Normalised_NETseq_Bins_MatrixC(classes==n, :) ;
            
            prevClasses = Normalised_NETseq_Bins_MatrixC(classes<n, :);
            ...
            Class_sorted_NET_seq_matrix = Normalised_NETseq_Bins_MatrixC(ix,:);
...
%         [sorted_classes, ix] = sort(classes);
%         Class_sorted_NET_seq_matrix2 = Normalised_NETseq_Bins_MatrixC(ix,:) ;
%         
%         Check_sorting_works = Normalised_NETseq_Bins_MatrixC(ix, :) == Class_sorted_NET_seq_matrix ;
%         
%         clear original_order
%         original_order(ix, :) = Class_sorted_NET_seq_matrix ; % unsorts the above, so orginal_order == Normalised_NETseq_Bins_MatrixC
...
%         NETseq_3primeexprnSelected_Normalised_Bins_Matrix_start = Normalised_NETseq_Bins_MatrixC_start(list_remaining_genes, :);
...
%         Classmetagenes5prime_end = zeros(size(Normalised_NETseq_Bins_MatrixC_start, 2), K);
%         Cumulative_Classize = cumsum(classize_matrix0) ;
%         for q = 2:K+1
%             % Classmetagenes5prime_end(:, (q-1)) = sum(NETseq_matrix_3prime_Class_sorted_fiveprime_end( (classize_matrix((q-1),1)+1):(classize_matrix((q-1),1)+classize_matrix(q,1)), :)))/classize_matrix(q, 1) ;
