...
save(filenameratios, 'ratios_matrix', '-mat')

toc


%% Histogram plot needs to incorporate clustering 
...
%         for clctr = 2:K
%             
%             avg_ratios(1,clctr-1) = nanmean((approach_ratio_matrix(classes == (clctr-1)& isfinite(approach_ratio_matrix))));
%             avg_ratios(1,clctr) = nanmean((approach_ratio_matrix(classes == (clctr)& isfinite(approach_ratio_matrix))));
%             colnames(1,clctr-1) = append('Cluster', string(clctr-1));
%             colnames(1,clctr) = append('Cluster', string(clctr));
%             if clctr>2
%                 avg_ratios(2,clctr-2) = nanmean((approach_ratio_matrix(classes == (clctr-2) & isfinite(approach_ratio_matrix))));
%                 colnames(1,clctr-2) = append('Cluster', string(clctr-2));
%             end
%             
%             avg_ratios(2,clctr-1) = nanmean((termination_ratio_matrix(classes == (clctr-1)& isfinite(termination_ratio_matrix))));
%             avg_ratios(2,clctr) = nanmean((termination_ratio_matrix(classes == (clctr)& isfinite(termination_ratio_matrix))));
%             
%             if clctr>2
%                 avg_ratios(2,clctr-2) = nanmean((termination_ratio_matrix(classes == (clctr-2)& isfinite(termination_ratio_matrix))));
%             end
%             
%             avg_ratios(3,clctr-1) = nanmean((aftermath_ratio_matrix(classes == (clctr-1)& isfinite(aftermath_ratio_matrix))));
%             avg_ratios(3,clctr) = nanmean((aftermath_ratio_matrix(classes == (clctr)& isfinite(aftermath_ratio_matrix))));
%             
%             if clctr>2
%                 avg_ratios(3,clctr-2) = nanmean((aftermath_ratio_matrix(classes == (clctr-2)& isfinite(aftermath_ratio_matrix))));
%             end
%             
%         end
%         f = figure;
%         rownames = ["approach_ratio", "termination_ratio","aftermath_ratio"];
%         
...
%         fprintf(fid, 'approach_ratio     termination_ratio      aftermath_ratio\n');
%                 fprintf(fid, '%f %f %f\n', avg_ratios );
%                 fclose(fid);
%         
%      
