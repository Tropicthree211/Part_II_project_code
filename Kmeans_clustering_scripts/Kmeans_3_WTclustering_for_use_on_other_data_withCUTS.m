...
%time 
klist=1:1000;%the number of clusters to try
...
%ufdata = 'PROseqspt4del';
 for K= 2:3
    for repeat = 1:3
        ...
        ClassmetagenesnoCUTs = zeros(size(Normalised_NETseq_Bins_Matrix, 2),K)
        
       classize_matrixnoCuts = zeros(K, 1);
       classize_matrix0noCuts = zeros(K+1, 1) ;
       classize_matrix_stringnoCuts = strings(K, 1);
        for n =1:K
...
%             index to be pulled out that can sort 
            ...
            classgenenames = filtered_gene_names(classes == (n));
                ...
                %metagene for each class without CUTS
                Normalised_NETseq_Bins_MatrixnoCuts = Normalised_NETseq_Bins_Matrix;
                classesnoCuts = classes ;
                gctr=0;
                for gctr = size(Normalised_NETseq_Bins_Matrix,1):-1:1
                    
                    splitname = split(gene_names_list(gctr,1), '');
                    if splitname{2} ~= 'Y'
                        Normalised_NETseq_Bins_MatrixnoCuts(gctr,:) = [] ;
                        classesnoCuts(gctr,:) = [];
                        
                    end
                end
                
                ClassnnoCuts = Normalised_NETseq_Bins_MatrixnoCuts(classesnoCuts==n, :) ;
            
            prevClassesnoCuts = Normalised_NETseq_Bins_MatrixnoCuts(classesnoCuts<n, :);
            
            classizenoCuts = size(ClassnnoCuts, 1);
            classize_matrixnoCuts((n), 1) =  classizenoCuts 
            classize_matrix0noCuts((n+1), 1) = classizenoCuts; %this allows the 5' end clustering to work outside of the loop 
            
            prevclassizesnoCuts = size(prevClassesnoCuts,1);
            [sorted_classesnoCuts, iz] = sort(classesnoCuts);
            Class_sorted_NET_seq_matrixnoCuts = Normalised_NETseq_Bins_MatrixnoCuts(iz,:);
%             Class_sorted_NET_seq_matrix(prevclassizes+1:classize+prevclassizes,
%             :) = Classn ; old way of sorting use sort function allows
%             index to be pulled out that can sort 
            
            classize_matrix_stringnoCuts(n, 1) = string(classize_matrixnoCuts(n, 1) ) ;
            
            
            
            %metagene for each class
            ClassmetagenesnoCUTs(:, n) = sum(Class_sorted_NET_seq_matrixnoCuts(prevclassizesnoCuts+1:classizenoCuts+prevclassizesnoCuts, :))/classizenoCuts ;
%             classgenenames = filtered_gene_names(classes == (n));
...
%%
filenamenoCuts = append('noCutsvswithCUTs', clusters, string(ybefore), 'before', string(xafter), 'after', string(K), 'clusters', string(repeat));
%         figure
        plot(ClassmetagenesnoCUTs)
        title(filenamenoCuts) ;
        legend(classize_matrix_stringnoCuts)
...
%         
%         
%% Remove Cuts in same clusters and take metagenes again. 
