...
figure 
plot(metagene_forward')
title(append(mutant, datatype1, 'metagene forward'))
filenamei = append(datatype1, mutant, 'forwardmetagene');
saveas(gcf, filenamei, 'jpeg')
figure
plot(metagene_reverse')
title(append(mutant, datatype1, 'metagene reverse'))
filenamei = append(datatype1, mutant, 'reversemetagene');
saveas(gcf, filenamei, 'jpeg')
%% combine reverse and forward matrices
NET_Seq_Matrix1 = vertcat(NET_Seq_matrix_forward,NET_Seq_matrix_reverse) ;
...
figure 
plot(metagene_forward2)
title(append(mutant, datatype1, 'metagene forward repeat'))
figure
filenamei = append(datatype1, mutant, 'forwardrepeatmetagene');
saveas(gcf, filenamei, 'jpeg')
plot(metagene_reverse2)
title(append(mutant, datatype1, 'metagene reverse repeat'))
filenamei = append(datatype1, mutant, 'reverserepeatmetagene');
saveas(gcf, filenamei, 'jpeg')
%% combine reverse and forward matrices
NET_Seq_Matrix2 = vertcat(NET_Seq_matrix_forward2,NET_Seq_matrix_reverse2) ;
...
%% Normalise no negative NETseq Matrix
Normalised_NETseq_Matrix = Shape_normalisation_function(NETseq_matrix_no_negative); %only works if shape normalisation function is in current folder, need to find out how to specify path to function but for now will leave
NETseq_3exprn_selected_filtered_normalised = Normalised_NETseq_Matrix(list_remaining_genes,:);
...
%  end
 ...
 metagene_Normalised = zeros(1, length_pulled_out);
for gctr= 1:size(NETseq_3exprn_selected_filtered_normalised, 1)
    metagene_Normalised = metagene_Normalised + NETseq_3exprn_selected_filtered_normalised(gctr, :) ;
end
metagene_Normalised = metagene_Normalised ./ size(NETseq_3exprn_selected_filtered_normalised, 1) ;

%% plot normalised metagene 
figure 
plot(metagene_Normalised)
title(append(mutant, datatype1, 'normalised metagene'))
filenamei = append(datatype1, mutant, 'normalisedmetagene');
saveas(gcf, filenamei, 'jpeg')
%% NaN to zeros 
% Normalised_NETseq_Matrix(isnan(Normalised_NETseq_Matrix)) = 0; converts
% Nans to zeros rather than removing
...
figure
plot(meta_gene)
filenameg = append('Unfiltered', datatype1, mutant, annotations, '.mat');
save(filenameg, 'Normalised_ufdata_Bins_Matrix')
unix(append('mv ', filenameg, ' Unfiltered_data'))

%% Now pull out the average signal for each gene across the 3 windows at the 3' end and generate ratio to -300to-200 signal
%Have moved to after second dataset so that same genes as in WT are considered
% Will nee dto chnNETseq_3exprn_selected_ufdata = Normalised_ufdata_Bins_Matrix(list_remaining_genes, :);age the input from NETseq_matrix_no_negative to a 
% filtered version

raw_NETseq_3exprn_selected_ufdata = NETseq_matrix_no_negative(list_remaining_genes, :);

%Pull out the specific windows from the above code 
approach_window_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1), 100);
termination_window_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1), 100);
aftermath_window_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1), 100);
control_elongation_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1), 100);


for gctr = 1:1:size(raw_NETseq_3exprn_selected_ufdata) 
    %need to ask jane about best exact window locations these are
    %placeholders to test the code, done have incorporated
    approach_window_matrix(gctr, :) = raw_NETseq_3exprn_selected_ufdata(gctr, 201:300);
    
    termination_window_matrix(gctr, :) =raw_NETseq_3exprn_selected_ufdata(gctr, 301:400);
    
    aftermath_window_matrix(gctr, :) = raw_NETseq_3exprn_selected_ufdata(gctr, 501:600);
    
    control_elongation_matrix(gctr,:) = raw_NETseq_3exprn_selected_ufdata(gctr,1:100);
   
end


%Take means of the signal in each window 
approach_mean_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
termination_mean_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
aftermath_mean_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
control_mean_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
...
%now using unfiltered data pulled out by WT filtering may need to use
%nanmean. 

%Comparability of using same genes should be worth it. 

approach_mean_matrix(gctr,1) = nanmean(approach_window_matrix(gctr,:));

termination_mean_matrix(gctr,1) = nanmean(termination_window_matrix(gctr,:));

aftermath_mean_matrix(gctr,1) = nanmean(aftermath_window_matrix(gctr,:));

control_mean_matrix(gctr,1) = nanmean(control_elongation_matrix(gctr,:));

end 

%Divide through window mean by control mean to get ratio for each window in
%each gene in 3 large matrices 
approach_ratio_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
termination_ratio_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
aftermath_ratio_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
control_ratio_matrix = zeros(size(raw_NETseq_3exprn_selected_ufdata, 1),1);
...
%% Created a logical that removes the top 10 and bottom 10 percent

%PrePolyA ratio
[sorted_approach_ratio_matrix, ix2] = sort(approach_ratio_matrix);

x = zeros(size(sorted_approach_ratio_matrix));
%make logical with zeros at top and bottom 10percent
x(round(size(sorted_approach_ratio_matrix,1)/10):1:round(9*size(sorted_approach_ratio_matrix)/10)) = 1;

%reverse sort by PrePolyA sorting to find large outliers and remove without
%deleting 
unsorted = 1:length(1:size(sorted_approach_ratio_matrix,1));
NewInd(ix2) = unsorted;

Removetopbottom10pclogical = x(NewInd);




%PostPolyA ratio
[sorted_termination_ratio_matrix, ix3] = sort(termination_ratio_matrix);

y = zeros(size(sorted_termination_ratio_matrix));

y(round(size(sorted_termination_ratio_matrix,1)/10):1:round(9*size(sorted_termination_ratio_matrix)/10)) = 1;

NewInd2(ix3) = unsorted;

Removetopbottom10pclogical2 = y(NewInd2);
...
%PostTerm ratio
[sorted_aftermath_ratio_matrix, ix4] = sort(aftermath_ratio_matrix);

z = zeros(size(sorted_aftermath_ratio_matrix));

z(round(size(sorted_aftermath_ratio_matrix,1)/10):1:round(9*size(sorted_aftermath_ratio_matrix)/10)) = 1;

NewInd3(ix4) = unsorted;

Removetopbottom10pclogical3 = z(NewInd3);



%% combine all the ratios into big matrix and save 
ratios_matrix = [approach_ratio_matrix, termination_ratio_matrix, aftermath_ratio_matrix];

filenameratios = append(datatype1,mutant,'ratios.mat');
save(filenameratios, 'ratios_matrix', 'Removetopbottom10pclogical','Removetopbottom10pclogical2','Removetopbottom10pclogical3', '-mat')
...
title('Elbow method clustering evaluation')
...
ufdata = append(datatype1, mutant, annotations) 
...
%ufdata = 'PROseqspt4del';
 for K= 2:3
    for repeat = 1:3
        ...
        save(append(datatype, mutant1,string(K), 'clusters', string(repeat), 'clustering.mat','classes', '-mat')) 
...
%             index to be pulled out that can sort 
            ...
            classgenenames = filtered_gene_names(classes == (n));
                ...
                fid = fopen(append(directory,datatype, mutant1 ,datatype1, mutant , string(n), 'cluster_of', string(K),'rep', string(repeat) , 'names','.txt' ) , 'w' );
                fprintf(fid, '%s\n', classgenenames);
                fclose(fid);
...
%% save boxplot
        ...
        w = 0;
        colnames = strings(nchoosek(K,2),1);
        for clctr = 2:K
        [p, h] = ranksum(gene_sizes_matrix_filtered(classes==(clctr-1)), gene_sizes_matrix_filtered(classes == clctr));
        w=w+1;
        size_ranksums(1, w) = p ;
        size_ranksums(2, w) = h;
        %name rows as part of for loop
        colnames(clctr-1,1) = append('Cluster',string(clctr-1),' vs Cluster', string(clctr));
        
        if clctr > 2 
            [q, i] = ranksum(gene_sizes_matrix_filtered(classes==(clctr-2)), gene_sizes_matrix_filtered(classes == clctr));
            w=w+1;
            size_ranksums(1,w) = q ;
            size_ranksums(2,w) = i;
            colnames(clctr-2,1) = append('Cluster',string(clctr-2),' vs Cluster', string(clctr));
        end
        end
        ...
        f = figure ;
        z = uitable(f, 'Data', size_ranksums, 'ColumnName', colnames, 'RowName', rownames)
        z.Position(3:4) = z.Extent(3:4);
        %% save table and textfile
        filenamew = append(filename, 'expression_ranksums');
        ...
        %% Wilcoxon rank sum test to compare the medians of the two clusters in the boxplot
        expression_ranksums = zeros(2, nchoosek(K,2));
        w = 0;
        colnames = strings(nchoosek(K,2),1);
        for clctr = 2:K
        [p, h] = ranksum(gene_expression_matrix_filtered(classes==(clctr-1)), gene_expression_matrix_filtered(classes == clctr));
        w=w+1;
        expression_ranksums(1, w) = p ;
        expression_ranksums(2, w) = h;
       colnames(clctr-1,1) = append('Cluster',string(clctr-1),' vs Cluster', string(clctr));
        if clctr > 2 
            [q, i] = ranksum(gene_expression_matrix_filtered(classes==(clctr-2)), gene_expression_matrix_filtered(classes == clctr));
            w=w+1;
            expression_ranksums(1,w) = q ;
            expression_ranksums(2,w) = i;
            colnames(clctr-2,1) = append('Cluster',string(clctr-1),' vs Cluster', string(clctr));
        end
        end
        ...
        f = figure ;
        z = uitable(f, 'Data', expression_ranksums, 'ColumnName', colnames, 'RowName', rownames)
        z.Position(3:4) = z.Extent(3:4);
        %% save table and textfile
        filenamew = append(filename, 'expression_ranksums');
        saveas(gcf, filenamew, 'fig')
        ...
        saveas(gcf, filenamex, 'jpeg')
          ...
          %% Wilcoxon rank sum test to compare the medians of the two clusters in the boxplot
        singleness_ranksums = zeros(2, nchoosek(K,2));
        w = 0;
        colnames = strings(nchoosek(K,2),1);
        for clctr = 2:K
        [p, h] = ranksum(gene_singleness_matrix_filtered(classes==(clctr-1)), gene_singleness_matrix_filtered(classes == clctr));
        w=w+1;
        singleness_ranksums(1, w) = p ;
        singleness_ranksums(2, w) = h;
         colnames(clctr-1,1) = append('Cluster',string(clctr-1),' vs Cluster', string(clctr));
        if clctr > 2 
            [q, i] = ranksum(gene_singleness_matrix_filtered(classes==(clctr-2)), gene_singleness_matrix_filtered(classes == clctr));
            w=w+1;
        ...
        f = figure ;
        z = uitable(f, 'Data', singleness_ranksums, 'ColumnName', colnames, 'RowName', rownames)
        z.Position(3:4) = z.Extent(3:4);
        %% save table and textfile
        filenamew = append(filename, 'singleness_ranksums');
        saveas(gcf, filenamew, 'fig')
        saveas(gcf,filenamew, 'jpeg')
        ...
        saveas(gcf, filenamez, 'jpeg')
          ...
          %% Wilcoxon rank sum test to compare the medians of the two clusters in the boxplot
        mTIFsingleness_ranksums = zeros(2, nchoosek(K,2));
        colnames = strings(nchoosek(K,2),1);
        w = 0;
        for clctr = 2:K
        [p, h] = ranksum(gene_mTIFsingleness_matrix_filtered(classes==(clctr-1)), gene_mTIFsingleness_matrix_filtered(classes == clctr));
        w=w+1;
        mTIFsingleness_ranksums(1, w) = p; %might need to switch around orientation of these to get nice table.
        mTIFsingleness_ranksums(2, w) = h;
        colnames(clctr-1,1) = append('Cluster',string(clctr-1),' vs Cluster', string(clctr));
        
        if clctr > 2 
            [q, i] = ranksum(gene_mTIFsingleness_matrix_filtered(classes==(clctr-2)), gene_mTIFsingleness_matrix_filtered(classes == clctr));
            w=w+1;
        ...
        %% make table instead of text file
        colnames = ["p value", "result"];
        
        f = figure ;
        z = uitable(f, 'Data', mTIFsingleness_ranksums, 'ColumnName', colnames, 'RowName', rownames);
        z.Position(3:4) = z.Extent(3:4);
        %% save table and textfile
        filenamew = append(filename, 'mTIFsingleness_ranksums');
        saveas(gcf, filenamew, 'fig')
        saveas(gcf,filenamew, 'jpeg')
        ...
        %% Calculate average approach termination and aftermath ratios for each cluster make into nice table 
        avg_ratios = zeros(3,K);
        colnames = strings(1,K);
        for clctr = 2:K
            
            avg_ratios(1,clctr-1) = nanmean((approach_ratio_matrix(classes == (clctr-1) & Removetopbottom10pclogical == 1)));%need to add condition to remove top and bottom 10% 
            avg_ratios(1,clctr) = nanmean((approach_ratio_matrix(classes == (clctr) & Removetopbottom10pclogical == 1)));
            colnames(1,clctr-1) = append('Cluster', string(clctr-1));
            colnames(1,clctr) = append('Cluster', string(clctr));
            if clctr>2
                avg_ratios(2,clctr-2) = nanmean((approach_ratio_matrix(classes == (clctr-2)  & Removetopbottom10pclogical == 1)));
                colnames(1,clctr-2) = append('Cluster', string(clctr-2));
            end
            
            avg_ratios(2,clctr-1) = nanmean((termination_ratio_matrix(classes == (clctr-1) & Removetopbottom10pclogical2 == 1)));
            avg_ratios(2,clctr) = nanmean((termination_ratio_matrix(classes == (clctr) & Removetopbottom10pclogical2 == 1)));
            
            if clctr>2
                avg_ratios(2,clctr-2) = nanmean((termination_ratio_matrix(classes == (clctr-2) & Removetopbottom10pclogical2 == 1)));
            end
            
            avg_ratios(3,clctr-1) = nanmean((aftermath_ratio_matrix(classes == (clctr-1) & Removetopbottom10pclogical3 == 1)));
            avg_ratios(3,clctr) = nanmean((aftermath_ratio_matrix(classes == (clctr) & Removetopbottom10pclogical3 == 1)));
            
            if clctr>2
                avg_ratios(3,clctr-2) = nanmean((aftermath_ratio_matrix(classes == (clctr-2) & Removetopbottom10pclogical3 == 1)));
            end
            
        end
        f = figure;
        rownames = ["Pre-PolyA ratio", "Post-PolyA ratio","PostTerm_ratio"];
        
        z = uitable(f,'Data', avg_ratios, 'ColumnName', colnames, 'RowName',rownames);
        z.Position(3:4) = z.Extent(3:4);
        
        %% save all the individual values matrices, the average matrix and the uitable
        filenameb = append(filename, 'ratio_table');
        saveas(gcf, filenameb, 'jpeg')
        saveas(gcf, filenameb, 'fig')
        
%         save(
        %% output to text file
        
        fid = fopen(append(directory, mutant1, datatype, mutant, datatype1, string(K),'clusters',string(repeat),'repeat','ratios.txt' ) , 'w' );
        fprintf(fid, 'PrePolyA_ratio     PostPolyA_ratio      PostTerm_ratio\n');
                fprintf(fid, '%f %f %f\n', avg_ratios );
                fclose(fid);
        ...
        for clctr = 2:1:K
            figure
            histogram(approach_ratio_matrix(classes == clctr-1 & Removetopbottom10pclogical == 1))
            filenameh = append(filename,'cluster',string(clctr-1), 'PrePolyA_hist');
            title(append('Cluster ',string(clctr-1),'PrePolyA Ratio Histogram'))
            ylabel('Number of genes')
            xlabel('PrePolyA Ratio')
            saveas(gcf, filenameh, 'jpeg')
            saveas(gcf, filenameh, 'fig')
            
            
            figure
            histogram(termination_ratio_matrix(classes == clctr-1 & Removetopbottom10pclogical2 == 1))
            filenameh = append(filename,'cluster',string(clctr-1), 'PostPolyA_hist');
            title(append('Cluster ',string(clctr-1),'PostPolyA Ratio Histogram'))    
            ylabel('Number of genes')
            xlabel('PostPolyA _ratio')
            saveas(gcf, filenameh, 'jpeg')          
            saveas(gcf, filenameh, 'fig')
            
            figure
            histogram(aftermath_ratio_matrix(classes == clctr-1 & Removetopbottom10pclogical3 == 1))
            filenameh = append(filename,'cluster',string(clctr-1), 'PostTerm_hist');
            title(append('Cluster ',string(clctr-1),'Post Termination Ratio Histogram'))  
            ylabel('Number of genes')
             xlabel('Post Termination Ratio')
            saveas(gcf, filenameh, 'jpeg')
            saveas(gcf, filenameh, 'fig')
            
            figure
            histogram(approach_ratio_matrix(classes == clctr & Removetopbottom10pclogical == 1))
            filenameh = append(filename,'cluster',string(clctr), 'PrePolyA_hist');
            title(append('Cluster ',string(clctr),'PrePolyA Ratio Histogram'))
            ylabel('Number of genes')
             xlabel('PrePolyA ratio')
            saveas(gcf, filenameh, 'jpeg')
            saveas(gcf, filenameh, 'fig')
            ...
            figure
            histogram(termination_ratio_matrix(classes == clctr & Removetopbottom10pclogical2 == 1))
            filenameh = append(filename,'cluster', string(clctr),'PostPolyA_hist');
            title(append('Cluster ',string(clctr),'PostPolyA Ratio Histogram'))    
            ylabel('Number of genes')
            xlabel('PostPolyA _ratio')            
            saveas(gcf, filenameh, 'jpeg')
            saveas(gcf, filenameh, 'fig')
            ...
            figure
            histogram(aftermath_ratio_matrix(classes == clctr & Removetopbottom10pclogical3 == 1))
            filenameh = append(filename,'cluster',string(clctr), 'PostTerm_hist');
            title(append('Cluster ',string(clctr),'Post Termination Ratio Histogram'))
            ylabel('Number of genes')
            xlabel('Post Termination Ratio')
            saveas(gcf, filenameh, 'jpeg')
            saveas(gcf, filenameh, 'fig')
            ...
            if clctr>2
                ...
                figure
                histogram(approach_ratio_matrix(classes == clctr-2 & Removetopbottom10pclogical == 1))
                filenameh = append(filename,'cluster',string(clctr-2), 'PrePolyA_hist');
                title(append('Cluster ',string(clctr-2),'PrePolyA Ratio Histogram'))
                ylabel('Number of genes')
                xlabel('PrePolyA ratio')
                saveas(gcf, filenameh, 'jpeg')
                saveas(gcf, filenameh, 'fig')
                ...
                figure
                histogram(termination_ratio_matrix(classes == clctr-2 & Removetopbottom10pclogical2 == 1))
                filenameh = append(filename,'cluster',string(clctr-2), 'PostPolyA_hist');
                title(append('Cluster ',string(clctr-2),'PostPolyA Ratio Histogram'))
                saveas(gcf, filenameh, 'jpeg')
                saveas(gcf, filenameh, 'fig')
                ylabel('Number of genes')
                xlabel('PostPolyA _ratio') 
                ...
                figure
                histogram(aftermath_ratio_matrix(classes == clctr-2 & Removetopbottom10pclogical3 == 1))
                filenameh = append(filename,'cluster',string(clctr-2), 'PostTerm_hist');
                title(append('Cluster ',string(clctr-2),'Post Termination Ratio Histogram'))
                ylabel('Number of genes')
                xlabel('Post Termination Ratio')
                saveas(gcf, filenameh, 'jpeg')
                saveas(gcf, filenameh, 'fig')
     ...
     %% Ranksums to compare medians of the two groups
   
     % PrePolyA window
     PrePolyA_ratio_ranksums = zeros(2, nchoosek(K,2));
        colnames = strings(nchoosek(K,2),1);
        w = 0;
        for clctr = 2:K
        [p, h] = ranksum(approach_ratio_matrix(classes==(clctr-1)& Removetopbottom10pclogical == 1 ), approach_ratio_matrix(classes == clctr & Removetopbottom10pclogical == 1 ));
        w=w+1;
        ...
        if clctr > 2 
            [q, i] = ranksum(approach_ratio_matrix(classes==(clctr-2) & Removetopbottom10pclogical == 1), approach_ratio_matrix(classes == clctr) & Removetopbottom10pclogical == 1);
            w=w+1;
...
% for z = 2:1:n+1
% classgenenames = filtered_gene_names(classes == (z-1));
% directory = '/home/orie3770/';
% fid = fopen(append(directory, mutant1 ,datatype, mutant, datatype, string(z-1), 'clustof', string(n), 'clusters', string(repeat),'rep' , 'names','.txt' ) , 'w' );
% fprintf(fid, '%s\n', classgenenames);
% fclose(fid);
% end
...
unix(append('mv ', datatype, mutant1, '* ', JOB))
unix(append('mv ', mutant, datatype1, '* ', JOB))
unix(append('mv ', datatype1, mutant, '* ', JOB))
unix(append('mv ', mutant1, datatype, '* ', JOB))
%unix(append('mv PROsppend('mv ', munix(append('mv ', datatype1, mutant, '* ', JOB))utant, datatype1, '* ', JOB))eq', mutant1, '* ', JOB))
